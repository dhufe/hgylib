...
HGParser::HGParser(const std::string &szFileName, uint8_t nExport)
    : szHFileName(szFileName), nDataOffset(0), nCurrentDataPointer(0),
      nExport(nExport), pFileBinaryHd(0) {}
...
void HGParser::parseFile(HGFileInfo **ppFileInfo) {
    ...
    try {
      ...
      if ((*ppFileInfo)->pUnits[i].compare("mm") == 0) {
        ...
        (*ppFileInfo)->pdStart[i] = (*ppFileInfo)->pdStart[i] * 1e-3;
                     ...
                     ->pUnits[i]
                     .compare((*ppFileInfo)->pUnits[i].size() - 1, 1, "s") ==
                 0) {
  ...
  for (size_t i = 0; i < (*ppFileInfo)->pDataTypes->size(); i++) {
    try {
      ...
      (*ppFileInfo)->pDataTypes->at(i).nDataOffset =
          hconfig.getIntValue(ss.str());
      ss.clear();
      ss.str("");
      // obtain number of bytes by multiplying number of samples with the number
      // of bytes per data word
      (*ppFileInfo)->pDataTypes->at(i).nBytes =
          (*ppFileInfo)->nSamples * nByteSize;
      // obtain culmulative number of bytes for the whole file
      (*ppFileInfo)->nBytes += (*ppFileInfo)->pDataTypes->at(i).nBytes;
      ss.clear();
      ss.str("");
      ss << "Measure" << i + 1 << ".Unit";
      (*ppFileInfo)->pDataTypes->at(i).szUnit =
          std::string(hconfig.getStringValue(ss.str()));
      ss.clear();
      ss.str("");
      ss << "Measure" << i + 1 << ".ScaleFactor";
      (*ppFileInfo)->pDataTypes->at(i).dScaling =
          hconfig.getDoubleValue(ss.str());
      ss.clear();
      ss.str("");
    } catch (int e) {
      if (e == 0) {
...
void HGParser::printExportTable(HGFileInfo **ppFileInfo) {
  ...
  try {
    ...
    oFile << "| Samplerate      " << "| " << dSampleRate << " MHz  |"
          << std::endl;
  } catch (HLibException /*&e*/) {
    // e.what();
  }
  ...
  for (size_t i = 0; i < (*ppFileInfo)->nCoordinates; ++i) {
    switch (i) {
    case 0:
      oFile << "| dy              " << "| " << (*ppFileInfo)->pdScale[i] * 1e3
            << " mm |" << std::endl;
      break;

    case 1:
      oFile << "| dx              " << "| " << (*ppFileInfo)->pdScale[i] * 1e3
            << " mm |" << std::endl;
      break;
    default:
      break;
  ...
  try {
    ...
    oFile << "| Highpass        " << "| " << dHighPass * 1e-3 << " kHz  |"
          << std::endl;
  } catch (HLibException /*&e*/) {
    // e.what();
  }
  ...
  try {
    ...
    oFile << "| Pulsewidth      " << "| " << dPulseWidth << " us |"
          << std::endl;
  } catch (HLibException /*&e*/) {
    // e.what();
  }
  ...
  try {
    ...
    oFile << "| Applied gain    " << "| " << dAppliedGain << " dB |"
          << std::endl;
  } catch (HLibException) {
  }
  ...
  try {
    ...
    oFile << "| Ascan    " << "| " << dStartTime * 1e6 << " - "
          << dEndTime * 1e6 << " us |" << std::endl;
  } catch (HLibException) {
  }
  ...
  try {
    ...
    oFile << "| Gate #1    " << "| " << dStartGate1 * 1e6 << " - "
          << dEndGate1 * 1e6 << " us |" << std::endl;
  } catch (HLibException) {
  }
...
void HGParser::closeFileBinary(void) { pFileBinaryHd->close(); }

ssize_t HGParser::getDataChunk(char *pcAmplitude, ssize_t nChunkSize,
                               ssize_t nOffset, ssize_t nMaxSize) {
  if (pcAmplitude == nullptr)
    return 0;
  ...
  try {
    // necessary !? we do not open and close the file anymore between two
    // readings pFileBinaryHd->seekg ( this->nCurrentDataPointer );
...
bool HGParser::data::iskey(const std::string &s) const { return count(s) != 0; }
