...
static const char *HGDataTypeStr[] = {"HGYVScan", "HGYBScan", "HGYCScan",
                                      "HGYDScan"};
...
struct HGDataType {
  ...
  HGDataType(eDataType _type)
      : DataType(_type), nBytes(0), nDataWordSize(0), nDataOffset(0),
        dScaling(0) {}

  std::string toString(void) const { return HGDataTypeStr[DataType]; }

  std::string toString(eDataType _type) const { return HGDataTypeStr[_type]; }
