...
MatioWrapper::MatioWrapper(const std::string& _FileName)
    : szFileName(_FileName), pMat(NULL), pMatVar(NULL) {}

bool MatioWrapper::writeData(const HGFileInfo* pDataInfo, const char* pcData) {
  if (szFileName.empty()) return false;

  if (pDataInfo == nullptr) return false;

  if (pcData == nullptr) return false;
  ...
  for (std::vector<HGDataType>::iterator pIter = pDataInfo->pDataTypes->begin();
       pIter != pDataInfo->pDataTypes->end(); ++pIter) {
    if (pSig == NULL) {
        ...
        for (auto y = 0; y < pDataInfo->pnDimension[1]; y++) {
          for (auto x = 0; x < pDataInfo->pnDimension[0]; x++) {
            switch ((*pIter).nDataWordSize) {
              default:
              case 2:
                pSig[pDataInfo->pnDimension[0] * y + x] =
                    (*pIter).dScaling *
                    (double)(pIData[pDataInfo->pnDimension[0] * y + x]);
                break;
              case 4:
                pSig[pDataInfo->pnDimension[0] * y + x] =
                    (*pIter).dScaling *
                    (double)(pI32Data[pDataInfo->pnDimension[0] * y + x]);
                break;
            }
          }
        }
          ...
          for (auto y = 0; y < pDataInfo->pnDimension[1]; y++) {
            for (auto x = 0; x < pDataInfo->pnDimension[0]; x++) {
              switch ((*pIter).nDataWordSize) {
                default:
                case 2:
                  pSig[pDataInfo->pnDimension[0] * pDataInfo->pnDimension[1] *
                           z +
                       pDataInfo->pnDimension[0] * y + x] =
                      (*pIter).dScaling *
                      (double)(pIData[pDataInfo->pnDimension[0] *
                                          pDataInfo->pnDimension[1] * z +
                                      pDataInfo->pnDimension[0] * y + x]);
                  break;
                case 4:
                  pSig[pDataInfo->pnDimension[0] * pDataInfo->pnDimension[1] *
                       ...
                       pDataInfo->pnDimension[0] * y + x] =
                      (*pIter).dScaling *
                      (double)(pI32Data[pDataInfo->pnDimension[0] *
                                            pDataInfo->pnDimension[1] * z +
                                        pDataInfo->pnDimension[0] * y + x]);
                  break;
              }
      ...
      pMatVar = Mat_VarCreate(ss.str().c_str(), MAT_C_DOUBLE, MAT_T_DOUBLE,
                              (int)(pDataInfo->nCoordinates), pSizes, pSig,
  ...
  for (auto i = 0; i < pDataInfo->nCoordinates; i++) {
    ...
    pdSizes[i] = (double)(pDataInfo->pdStart[i] +
                          pDataInfo->pnDimension[i] * pDataInfo->pdScale[i]);
  ...
  ppMatStructVars[0] = Mat_VarCreate("Scale", MAT_C_DOUBLE, MAT_T_DOUBLE, 2,
                                     sDim, (void*)(pDataInfo->pdScale), 0);
  ppMatStructVars[1] = Mat_VarCreate("Start", MAT_C_DOUBLE, MAT_T_DOUBLE, 2,
                                     sDim, (void*)(pDataInfo->pdStart), 0);
  ppMatStructVars[2] = Mat_VarCreate("Dimensions", MAT_C_DOUBLE, MAT_T_DOUBLE,
                                     2, sDim, (void*)(pnSizes), 0);
  ppMatStructVars[3] = Mat_VarCreate("Sizes", MAT_C_DOUBLE, MAT_T_DOUBLE, 2,
                                     sDim, (void*)(pdSizes), 0);
  ppMatStructVars[4] = Mat_VarCreate("Rotary", MAT_C_INT32, MAT_T_INT32, 2,
                                     sDim, (void*)(piRotary), 0);
  ppMatStructVars[5] = NULL;

  matvar_t* pMatStruct = Mat_VarCreate("recparm", MAT_C_STRUCT, MAT_T_STRUCT, 2,
                                       dims_struct, ppMatStructVars, 0);
