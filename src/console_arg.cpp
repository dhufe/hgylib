...
int ConArgs::parse_arguments_long(int argc, char* argv[], const char* cOptions,
                                  const struct option* strOption) {
  int c;
  int option_idx = 0;
  ...
  while (1) {
    ...
    switch (c) {
      case 0:
        ...
        if (strOption[option_idx].flag != 0) break;
  ...
  if (szInputFileName.empty()) return -1;
...
void ConArgs::usage(void) {
  ...
  std::cout << "Copyright " << '\xA9' << "Daniel Hufschlaeger (dhufe)"
            << std::endl;
  std::cout << std::endl;
  ...
  std::cout << "Usage: " << szApplicationName << " [-io]" << std::endl
            << std::endl;
  std::cout << "  -i (--infile)         input filename (HGY format)"
            << std::endl;
  std::cout << "  -o (--outfile)        output filename (MATLAB format)"
            << std::endl;
  std::cout << std::endl;
  std::cout
      << "  --export)             exports reconstruction parameters to markdown"
      << std::endl;
  std::cout << std::endl;
  std::cout << "  --verbose             verbose output information"
            << std::endl;
  std::cout << std::endl;
}
